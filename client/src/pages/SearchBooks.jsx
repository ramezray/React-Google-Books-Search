...
class SearchBooks extends Component {
  constructor(props) {
    super(props);
    this.state = { value: "", createDiv: <div />, booksArray: [] };
      ...
      .then(response => {
        this.setState({
          createDiv: response.data.items.map(books => (
            <div
              className="card p-2 mb-2 text-primary"
              key={books.id}
            >
              <span>
            ...
            {this.state.booksArray.push({title:books.volumeInfo.title, authors:books.volumeInfo.authors,description:books.volumeInfo.description,image:books.volumeInfo.imageLinks.thumbnail, link: books.volumeInfo.infoLink})}
              <button className="btn btn-primary my-2 my-sm-0 float-right" formMethod="POST" formAction={save.saveBook(this)} onClick={console.log(books.id)}>
                Save
              </button>
